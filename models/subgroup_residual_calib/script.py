...
def blend_predictions_3way(
    ...
    weights["extratrees"] = min(float(weights.get("extratrees", 0.0)), MAX_EXTRATREES_WEIGHT)
    total = float(weights.get("lgbm", 0.0)) + float(weights.get("catboost", 0.0)) + weights["extratrees"]
    if total <= 0:
        weights = {"lgbm": 0.12, "catboost": 0.83, "extratrees": 0.05}
        total = 1.0
    weights["lgbm"] = float(weights.get("lgbm", 0.0)) / total
    weights["catboost"] = float(weights.get("catboost", 0.0)) / total
    weights["extratrees"] = weights["extratrees"] / total
    pred = (
...
def apply_residual_corrector(
    ...
    max_abs_delta = float(corrector.get("max_abs_delta", 0.025))
    strength = float(corrector.get("strength", 0.0))
    correction = np.clip(corrector["model"].predict(residual_x), -max_abs_delta, max_abs_delta)
...
def apply_postprocess(
    ...
    postprocess: dict[str, object] | None,
) -> np.ndarray:
    current = apply_calibration(pred, calibration)
    if postprocess:
        current = apply_subgroup_calibration(
            x,
            current,
            postprocess.get("subgroup_calibration"),
        )
        current = apply_residual_corrector(
            x,
            current,
            postprocess.get("residual_corrector"),
        )
    return np.clip(current, 1e-6, 1 - 1e-6)
...
def main() -> None:
    ...
    if len(x_test):
        ...
        preds = apply_postprocess(x_test, preds, calibration, postprocess)
    else:
        preds = np.array([])
