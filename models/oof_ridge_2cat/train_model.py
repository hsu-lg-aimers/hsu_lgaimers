...
META_RIDGE_ALPHA = 700.0
DEFAULT_OOF_SEASONS = [2022, 2023, 2024]
META_CAT_COLS = [
    "top_bottom",
    "game_type",
    "base_state",
...
def make_meta_pipeline(cat_cols: list[str], num_cols: list[str]) -> Pipeline:
    preprocessor = ColumnTransformer(
        transformers=[
            (
                "cat",
                OneHotEncoder(handle_unknown="ignore", min_frequency=300),
                cat_cols,
            ),
            ("num", SimpleImputer(strategy="median"), num_cols),
        ],
    ...
    model = Ridge(alpha=META_RIDGE_ALPHA, solver="lsqr")
    return Pipeline([("pre", preprocessor), ("ridge", model)])
...
def strip_predict_unused_rng(model: object) -> None:
    estimators = []
    ...
    for estimator in estimators:
        if hasattr(estimator, "_feature_subsample_rng"):
            estimator._feature_subsample_rng = None


def strip_artifact_unused_rng(artifact: dict[str, object]) -> None:
    for model in artifact.get("models", {}).values():
        strip_predict_unused_rng(model)

    postprocess = artifact.get("postprocess", {})
    if isinstance(postprocess, dict):
        meta_residual = postprocess.get("meta_residual", {})
        if isinstance(meta_residual, dict) and "model" in meta_residual:
            strip_predict_unused_rng(meta_residual["model"])
...
def apply_train_variant(fit_df: pd.DataFrame, variant: str) -> tuple[pd.DataFrame, np.ndarray | None]:
    fit_df = fit_df.copy()
    sample_weight = None
...
def build_oof_postprocess(
    train: pd.DataFrame,
    trackman: pd.DataFrame,
    variant: str,
    oof_seasons: list[int],
    lgbm_device: str = "cpu",
    catboost_device: str = "cpu",
    gpu_device: str = "0",
    ...
    pred_parts = []
    fold_metrics = {}
    usable_seasons = [season for season in oof_seasons if season in set(train["season"])]

    for season in usable_seasons:
        fold_fit = train[train["season"] < season].copy()
        fold_valid = train[train["season"] == season].copy()
        fold_fit, sample_weight = apply_train_variant(fold_fit, variant)
        if len(fold_fit) < 10000 or len(fold_valid) < 1000:
            print(f"Skip OOF season={season}: fit={len(fold_fit)} valid={len(fold_valid)}")
            continue

        rate_priors = make_rate_priors(fold_fit)
        fold_trackman = trackman[trackman["season"] < season].copy()
        fold_trackman_prior = build_trackman_prior(fold_trackman)
        x_fit = build_features(fold_fit, rate_priors, fold_trackman_prior)
        x_valid = build_features(fold_valid, rate_priors, fold_trackman_prior)
        ...
        lgbm_pred, cat_pred, cat_alt_pred, _ = predict_ensemble_raw(
            models,
            x_valid,
            feature_columns,
            cat_columns,
        ...
        blend_weights = fit_blend_weights_3way(lgbm_pred, cat_pred, cat_alt_pred, fold_valid[TARGET_COL])
        raw_pred = blend_predictions_3way(lgbm_pred, cat_pred, cat_alt_pred, blend_weights)
        calibration = fit_logit_brier_calibration(raw_pred, fold_valid[TARGET_COL])
        calibrated_pred = apply_calibration(raw_pred, calibration)
        meta_x, _, _ = make_meta_frame(
            x_valid,
            lgbm_pred,
            cat_pred,
            cat_alt_pred,
        ...
        meta_frames.append(meta_x)
        target_parts.append(fold_valid[TARGET_COL].reset_index(drop=True))
        pred_parts.append(pd.Series(calibrated_pred))
        fold_metrics[f"oof_{season}_calibrated"] = score_dict(fold_valid[TARGET_COL], calibrated_pred)
        fold_metrics[f"oof_{season}_blend"] = blend_weights
        fold_metrics[f"oof_{season}_calibration"] = calibration

    if not meta_frames:
        print("Skip OOF postprocess: no usable OOF folds.")
        return {"meta_residual": {"enabled": False}, "order": ["global_logit_calibration", "oof_ridge_residual"]}, fold_metrics

    meta_x = pd.concat(meta_frames, ignore_index=True)
    y = pd.concat(target_parts, ignore_index=True)
    calibrated_pred = pd.concat(pred_parts, ignore_index=True).to_numpy(dtype="float64")
    cat_cols = [col for col in META_CAT_COLS if col in meta_x.columns]
    num_cols = [col for col in meta_x.columns if col not in cat_cols]
    model = make_meta_pipeline(cat_cols, num_cols)
    residual = y.to_numpy(dtype="float64") - calibrated_pred
    model.fit(meta_x, residual)
    raw_delta = np.clip(model.predict(meta_x), -META_MAX_ABS_DELTA, META_MAX_ABS_DELTA)

    def objective(strength: float) -> float:
        adjusted = np.clip(calibrated_pred + strength * raw_delta, 1e-6, 1 - 1e-6)
        return float(np.mean((adjusted - y.to_numpy(dtype="float64")) ** 2))

    result = minimize_scalar(objective, bounds=(0.0, 1.0), method="bounded", options={"xatol": 1e-5})
    strength = float(result.x if result.success else META_DEFAULT_STRENGTH)
    adjusted = np.clip(calibrated_pred + strength * raw_delta, 1e-6, 1 - 1e-6)
    fold_metrics["oof_stacked_calibrated"] = score_dict(y, calibrated_pred)
    fold_metrics["oof_stacked_residual"] = score_dict(y, adjusted)
    print(
        "OOF ridge residual: "
        f"rows={len(meta_x)} "
        f"features={meta_x.shape[1]} "
    ...
    return (
        {
            "meta_residual": {
                ...
                "max_abs_delta": META_MAX_ABS_DELTA,
                "alpha": META_RIDGE_ALPHA,
                "target": "season_oof_residual_after_global_calibration",
                "oof_seasons": usable_seasons,
            },
            "order": ["global_logit_calibration", "oof_ridge_residual"],
        },
        fold_metrics,
    )
...
def train_and_validate(
    ...
    fit_df, sample_weight = apply_train_variant(fit_df, variant)
    validation_oof_seasons = [season for season in oof_seasons if season < 2024]
    postprocess, oof_metrics = build_oof_postprocess(
        fit_df,
        trackman[trackman["season"] <= 2023].copy(),
        variant,
        validation_oof_seasons,
        lgbm_device,
        catboost_device,
        gpu_device,
    )
    ...
    calibrated_pred = apply_calibration(raw_pred, calibration)
    meta_pred = apply_meta_residual_corrector(
        x_valid,
        lgbm_pred,
        cat_pred,
        cat_alt_pred,
    ...
    metrics["oof_postprocess"] = oof_metrics
...
def main() -> None:
    ...
    parser.add_argument(
        "--oof-seasons",
        default=",".join(str(season) for season in DEFAULT_OOF_SEASONS),
        help="Comma-separated seasons used to train the OOF Ridge residual corrector.",
    )
    parser.add_argument(
        "--skip-final-oof-postprocess",
        action="store_true",
        help="Keep validation postprocess instead of rebuilding OOF postprocess on all train rows.",
    )
    parser.add_argument(
        "--variant",
        choices=[
    ...
    oof_seasons = parse_season_list(args.oof_seasons)
    print(
        "Training devices: "
    ...
    print(f"OOF seasons: {oof_seasons}")
    ...
    postprocess = {"meta_residual": {"enabled": False}, "order": ["global_logit_calibration", "oof_ridge_residual"]}
    validation_metrics = {}
    ...
    if not args.skip_final_oof_postprocess and not args.validate_only:
        print("Build final OOF Ridge postprocess...")
        postprocess, final_oof_metrics = build_oof_postprocess(
            train,
            trackman,
            args.variant,
            oof_seasons,
        ...
        validation_metrics["final_oof_postprocess"] = final_oof_metrics
    ...
    strip_artifact_unused_rng(artifact)
