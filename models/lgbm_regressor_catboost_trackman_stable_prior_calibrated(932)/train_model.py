...
def make_lgbm_pipeline(feature_columns: list[str]) -> Pipeline:
...
def fit_ensemble(
    ...
    lgbm_model = make_lgbm_pipeline(feature_columns)
    cat_model = make_catboost_model()
...
def train_and_validate(
    ...
    models = fit_ensemble(x_fit, fit_df[TARGET_COL], feature_columns, cat_columns, sample_weight)
...
def main() -> None:
    ...
    cat_columns = [col for col in CAT_COLS if col in feature_columns]
    models = fit_ensemble(x_train, train[TARGET_COL], feature_columns, cat_columns)
