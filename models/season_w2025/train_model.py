...
def make_2025_season_weights(df: pd.DataFrame) -> np.ndarray:
    weights = df["season"].map(SEASON_WEIGHTS_2025).fillna(1.0).to_numpy(dtype="float64")
...
def train_and_validate(
    ...
    use_season_weighting: bool = True,
    lgbm_device: str = "cpu",
    catboost_device: str = "cpu",
    gpu_device: str = "0",
    ...
    if use_season_weighting:
        season_weight = make_2025_season_weights(fit_df)
        sample_weight = season_weight if sample_weight is None else sample_weight * season_weight
    ...
    metrics["variant"] = {
        ...
        "season_weighting_used": bool(use_season_weighting),
        "season_weights_2025": SEASON_WEIGHTS_2025,
    }
...
def main() -> None:
    ...
    parser.add_argument("--no-season-weighting", action="store_true")
    ...
    use_season_weighting = not args.no_season_weighting
    ...
    print(f"Season weighting: {use_season_weighting} weights={SEASON_WEIGHTS_2025}")
    ...
    cat_columns = [col for col in CAT_COLS if col in feature_columns]
    final_sample_weight = make_2025_season_weights(train) if use_season_weighting else None
    models = fit_ensemble(
        x_train,
        train[TARGET_COL],
        feature_columns,
    ...
    artifact = {
        ...
        "season_weighting": {
            "used": bool(use_season_weighting),
            "weights": SEASON_WEIGHTS_2025,
        },
        "training_devices": {
