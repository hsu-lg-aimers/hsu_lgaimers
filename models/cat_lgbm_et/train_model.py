...
def fit_blend_weights_3way(
    ...
    result = minimize(
        objective,
        x0=np.array([0.10, 0.80, 0.10]),
        method="SLSQP",
        bounds=[(0.0, 1.0), (0.0, 1.0), (0.0, 1.0)],
        constraints=[{"type": "eq", "fun": lambda w: float(np.sum(w) - 1.0)}],
        options={"maxiter": 300, "ftol": 1e-12},
    )
    weights = result.x if result.success else np.array([0.10, 0.80, 0.10])
...
def train_and_validate(
    ...
    if train["season"].nunique() < 2 or 2024 not in set(train["season"]):
        ...
        return (
            {"lgbm": 0.10, "catboost": 0.80, "extratrees": 0.10, "success": True, "objective": np.nan},
...
def main() -> None:
    ...
    blend_weights = {"lgbm": 0.10, "catboost": 0.80, "extratrees": 0.10, "success": True, "objective": np.nan}
