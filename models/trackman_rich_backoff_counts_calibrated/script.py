...
def build_features(
    ...
    rich_trackman_group_cols: list[str] | None = None,
) -> pd.DataFrame:
    ...
    if rich_trackman_prior is not None:
        rich_group_cols = (
            rich_trackman_group_cols
            if rich_trackman_group_cols is not None
            else RICH_TRACKMAN_GROUP_COLS
        )
        x = x.merge(rich_trackman_prior, on=rich_group_cols, how="left")
        x = x.copy()
...
def main() -> None:
    ...
    rich_trackman_prior = deserialize_trackman_prior(artifact.get("rich_trackman_prior"))
    rich_trackman_group_cols = artifact.get("rich_trackman_group_cols")
    ...
    if len(x_test):
        ...
        preds = blend_predictions(lgbm_pred, cat_pred, lgbm_weight)
